...
def getColumnNames():
    ...
    latName, lonName = input("Enter column name for latitude: "), input(
        "Enter column name for longitude: "
    )
...
def getLocale():
    ...
    lat, lon = float(input("Enter current latitude: ")), float(
        input("Enter current longitude: ")
    )
...
def computeDist(x1, y1, x2, y2):
    ...
    # <-- placeholder, can remove once defined.
    d = (x1 - x2) ** 2 + (y1 - y2) ** 2
...
def dotAllPoints(cMap, df, latCol, lonCol):
    ...
    for i, row in df.iterrows():
        folium.CircleMarker(
            location=[row[latCol], row[lonCol]], radius=4, color="red"
        ).add_to(cMap)
...
def markAndFindClosest(cMap, df, latCol, lonCol, cLat, cLon):
    ...
    # Find closest marker:
    df["Dist"] = df[[latCol, lonCol]].apply(
        lambda row: computeDist(*row, cLat, cLon), axis=1
    )
    minRow = df[df["Dist"] == df["Dist"].min()]

    # Make a marker for the closest point:
    folium.Marker(
        location=[float(minRow[latCol]), float(minRow[lonCol])], popup="Closest"
    ).add_to(cMap)
    # Make a marker for the starting point
    folium.Marker(
        location=[cLat, cLon], popup="You Are Here", icon=folium.Icon(color="red")
    ).add_to(cMap)
...
def main():
    ...
    lat, lon = getLocale()
    cityMap = folium.Map(location=[lat, lon],
